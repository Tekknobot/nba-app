...
module.exports = async function handler(req, res) {
  // --- CORS (allow your React app to call this from any origin in dev/prod) ---
  res.setHeader("Access-Control-Allow-Origin", "*");           // or set your exact domain
  ...
  if (!season || !/^\d{4}$/.test(String(season))) {
    return res.status(400).end(JSON.stringify({
      error: "bad_request",
      detail: "season must be a 4-digit year (e.g., 2025)"
    }));
  }
  ...
  if (!ids.length) {
    return res.status(400).end(JSON.stringify({
      error: "bad_request",
      detail: "at least one valid player_ids[] required"
    }));
  }
